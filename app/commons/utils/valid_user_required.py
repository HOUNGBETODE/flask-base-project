    ...
    def decorator(func):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if not token:
                    return CustomResponse.send_response(message="Token requis!", status_code=401)
                
                identity = get_jwt()
                jti = identity.get('jti')
                if RevokeTokenAdaptater.token_is_revoked(jti): 
                    return CustomResponse.send_response(message="Token revoqué ", status_code=401)
                ...
                if not user.is_verified:
                    return CustomResponse.send_response(message="Veuillez vérifier votre compte", status_code=403)
                
                return func(*args, **kwargs)

            except Exception as e:
                logger.error(f"Error on valid_user_required function decorator: {e}")
