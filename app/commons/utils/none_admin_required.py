    ...
    def decorator(func):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if (user.role in [UserRole.SUPER_ADMIN.value, UserRole.ADMIN.value]) or user.is_admin:
                    return CustomResponse.send_response(message="Action non autorisée", success=False, status_code=403)
                
                return func(*args, **kwargs)

            except Exception as e:
                logger.error(f"Error on none_admin_required function decorator: {e}")
